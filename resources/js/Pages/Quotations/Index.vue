...
const getStatusColor = (status) => {
    switch (status) {
        ...
        case 'production_ready': return 'bg-indigo-600';
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Quotations List</h2>
                <div class="text-sm text-gray-500">Home > CRM > Quotations</div>
            </div>
        </template>
                    ...
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="flex flex-col md:flex-row justify-between items-center mb-4 space-y-4 md:space-y-0">
                            ...
                            <div class="flex space-x-2">
                                <Link :href="route('quotations.create')" class="bg-cyan-500 hover:bg-cyan-600 text-white px-4 py-2 rounded-md text-sm font-medium flex items-center transition">
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-200 dark:bg-gray-700">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider border-r border-gray-300 dark:border-gray-600">Items</th>
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    <tr v-for="(quotation, index) in quotations.data" :key="quotation.id" :class="index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-700/50'" class="hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors duration-150">
                                        <td class="px-4 py-3 whitespace-nowrap text-sm border-r border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300">{{ quotation.quotation_date }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm border-r border-gray-200 dark:border-gray-700">
                                            <Link :href="route('quotations.show', quotation.id)" class="text-indigo-600 hover:underline font-medium">{{ quotation.quotation_no }}</Link>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm border-r border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300">{{ quotation.customer?.name }}</td>
                                            ...
                                            <div class="relative inline-block text-left group">
                                                ...
                                                <div class="origin-top-right absolute right-0 mt-0 pt-2 w-48 z-50 hidden group-hover:block hover:block">
                                                    <div class="rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 py-1">
                                                        <Link :href="route('quotations.show', quotation.id)" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">View</Link>
                                                        <Link :href="route('quotations.edit', quotation.id)" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Edit</Link>
                                                        <button @click="deleteQuotation(quotation.id)" class="block w-full text-left px-4 py-2 text-sm text-red-700 hover:bg-gray-100">Delete</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                        ...
                        <div class="mt-4 flex flex-col md:flex-row justify-between items-center">
                            <div class="text-sm text-gray-700 dark:text-gray-400 mb-4 md:mb-0">
                                Showing {{ (quotations.current_page - 1) * quotations.per_page + 1 }} to {{ Math.min(quotations.current_page * quotations.per_page, quotations.total) }} of {{ quotations.total }} entries
                            </div>
                            <div v-if="quotations.links.length > 3" class="flex justify-center">
                                <template v-for="(link, key) in quotations.links" :key="key">
