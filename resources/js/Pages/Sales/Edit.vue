...
const form = useForm({
    production_id: props.sale.production_id,
    customer_id: props.sale.customer_id,
    customer_name: props.sale.customer?.name, 
    type: props.sale.type,
    sale_date: new Date(props.sale.created_at).toISOString().slice(0, 10),
    items: props.sale.items.map(item => ({
        ...
        gsm: item.gsm,
        color: item.color,
        envelope_weight: item.envelope_weight,
        envelope_price: item.envelope_price,
        loop_weight: item.loop_weight,
        loop_price: item.loop_price,
        print_cost: item.print_cost,
    ...
    tax: props.sale.tax,
    other_costs: props.sale.other_costs,
    grand_total: props.sale.grand_total,
    advance_adjusted: props.sale.advance_adjusted,
...
const onProductionSelect = async () => {
    ...
    if (form.production_id === props.sale.production_id) {
        // If switching back to original production, we might still need to update prices if type changed
        // But for now, let's assume we re-fetch to be safe and consistent with Create logic
    }
    ...
    try {
        ...
        // Populate items from new production
...
watch(() => form.type, () => {
    if (form.production_id) {
        // If we are still using the same production as the original sale, we might not want to re-fetch
        // but re-calculating row-by-row might be enough if we have the unit prices?
        // Actually, the original items in Edit have price_per_piece from the DB.
        // If we switch to local, we might want to get the quotation unit price.
        // Re-running onProductionSelect will do that by fetching fresh data.
        onProductionSelect(); 
    }
});
...
const submit = () => {
    form.put(route('sales.update', props.sale.id), {
        onSuccess: () => Swal.fire('Updated!', 'Sale has been updated successfully.', 'success'),
        onError: (errors) => {
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Sale Invoice #{{ sale.sale_no }}</h2>
        </template>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6 bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                                <div class="md:col-span-2">
                                    ...
                                    <select v-model="form.production_id" @change="onProductionSelect" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                                        <option v-for="p in [sale.production, ...productions]" :key="p.id" :value="p.id">
                                            {{ p.production_no }} - {{ p.quotation?.customer?.name }}
                                        </option>
                                    </select>
                                </div>
                                <div>
                                ...
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border">
                                    ...
                                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200">
                                        <tr v-for="(item, index) in form.items" :key="index">
                                            ...
                                            <td class="px-1 py-1"><input v-model="item.envelope_weight" type="number" step="0.001" class="w-16 rounded-md border-gray-300 text-xs p-1 text-right" @input="calculateRow(index)"></td>
                                            <td class="px-1 py-1"><input v-model="item.envelope_price" type="number" step="0.01" class="w-16 rounded-md border-gray-300 text-xs p-1 text-right" @input="calculateRow(index)"></td>
                                            <td class="px-1 py-1"><input v-model="item.loop_weight" type="number" step="0.001" class="w-16 rounded-md border-gray-300 text-xs p-1 text-right" @input="calculateRow(index)"></td>
                                            <td class="px-1 py-1"><input v-model="item.loop_price" type="number" step="0.01" class="w-16 rounded-md border-gray-300 text-xs p-1 text-right" @input="calculateRow(index)"></td>
                                            <td class="px-1 py-1"><input v-model="item.print_cost" type="number" step="0.01" class="w-16 rounded-md border-gray-300 text-xs p-1 text-right" @input="calculateRow(index)"></td>
                                            <td class="px-1 py-1"><input v-model="item.sewing_cost" type="number" step="0.01" class="w-16 rounded-md border-gray-300 text-xs p-1 text-right" @input="calculateRow(index)"></td>
                                            
                                            <td class="px-1 py-1"><input v-model="item.quantity" type="number" step="1" class="w-16 rounded-md border-gray-300 text-xs p-1 text-right font-bold" @input="calculateRow(index)"></td>
