...
const form = useForm({
    ...
    total_cost: props.production.total_cost,
    note: props.production.note || '',
    items: props.production.items.map(item => ({
        ...
        envelope_weight: parseFloat(item.envelope_weight),
        envelope_price: parseFloat(item.envelope_price),
        loop_weight: parseFloat(item.loop_weight),
        loop_price: parseFloat(item.loop_price),
        print_cost: parseFloat(item.print_cost),
        sewing_cost: parseFloat(item.sewing_cost),
        
        price_per_piece: parseFloat(item.price_per_piece),
        total_price: parseFloat(item.total_price),
        
        wastage_kg: parseFloat(item.wastage_kg),
        wastage_piece: parseFloat(item.wastage_piece),
    }))
});
...
const grandTotalInfo = computed(() => {
    ...
    const advance = parseFloat(props.production.advance_amount) || 0;
    return {
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Production #{{ production.production_no }}</h2>
        </template>
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <form @submit.prevent="submit">
                            ...
                            <!-- Information Block -->
                            <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg mb-6 flex flex-wrap gap-6 text-sm">
                                <div><span class="font-bold">Quotation No:</span> {{ production.quotation?.quotation_no }}</div>
                                <div><span class="font-bold">Customer:</span> {{ production.quotation?.customer?.name }}</div>
                                <div><span class="font-bold">Advance Amount:</span> {{ production.advance_amount }}</div>
                            </div>
                            ...
                            <div class="mt-8 flex justify-end space-x-3">
                                <Link :href="route('production.index')" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300">Cancel</Link>
